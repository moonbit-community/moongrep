...
```mooncram
$ moonrun "$TESTDIR"/../moongrep.wasm -- dump --json --expr 'value +' 2>&1 >/dev/null
...
```mooncram
$ bash -c 'stdout=$(moonrun "$1" -- dump --json --expr "value +" 2>/dev/null); status=$?; test "$status" -eq 3 && test -z "$stdout"' _ "$TESTDIR"/../moongrep.wasm
```
...
```mooncram
$ moonrun "$TESTDIR"/../moongrep.wasm -- dump --json --help | grep '^  --json'
