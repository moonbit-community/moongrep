...
an outer expression, while `inside-toplevel` filters one MoonBit top-level item.
Both bind outer inline captures and then search the captured `__TARGET__`
...
a top-level item rather than an expression:
