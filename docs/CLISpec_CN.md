...
- `--exclude-dir <name-or-path>`：跳过匹配的遍历条目。
- `--exclude-rule <rule-id>`：禁用精确匹配的已加载规则 id。
...
- 最后一个 `--rule` 值生效。
- 每个 `--pattern` 都按顺序保留。
- 每个 `--exclude-dir` 和 `--exclude-rule` 都按顺序保留。
- 重复的布尔选项等同于只提供一次。
...
所有选中来源都会先加载，然后才应用 `--exclude-rule`。每个排除 id 都与组合后的
已加载规则 id 进行精确比较。每个请求的 id 都必须存在；第一个未知 id 会成为
状态码 2 的用法错误。

所有具有被排除 id 的规则都会在规则编译和扫描计划之前移除。重复的排除 id 按
...
比较使用完整且区分大小写的条目名称。这些排除应用于遍历中的子条目。显式选择的
扫描根即使以其中一个名称结尾，也仍会被检查。

### `--exclude-dir`

每个 `--exclude-dir` 值都会被规范化：先删除末尾的 `/` 字符但保留单独的 `/`，
再删除所有开头的 `./`。例如，`./vendor/` 变成 `vendor`，
`./src/generated///` 变成 `src/generated`。

访问一个子条目前，规范化后的排除值会与以下两个值精确比较：

- 子条目的名称；
- 经过相同规范化的已构造子路径。

任一个完整字符串匹配都会跳过该条目。比较把 glob 字符和 `..` 当作普通文本，
保留文件系统路径拼写和大小写。因为每一层都进行条目名称匹配，排除 `vendor` 会
跳过每个名为 `vendor` 的子条目。`src/generated` 这样的路径只匹配构造路径拼写
与它完全相同的子条目。
...
帮助、内嵌文档和 dump 输出也写入标准输出。当前 native 和 WebAssembly 前端会
把命令行用法诊断、未知文档诊断、dump 解析诊断，以及向上传播的规则加载、规则
编译、文件系统或其他运行期诊断作为普通命令文本写到标准输出。

### 标准错误和 Verbose 事件

扫描解析 warning 和无效 `#moongrep.skip` warning 总会写入标准错误。
`--verbose` 会添加以下标准错误事件：
...
位置使用从 1 开始的行号和 UTF-16 列号。起点包含在范围内，终点不包含在范围内。
...
JSON range 坐标与文本输出使用相同的从 1 开始的 UTF-16 坐标。
`range.start` 包含在范围内，`range.end` 不包含在范围内。
...
状态 1 表示选中的操作在加载、解析、编译或执行输入时失败。它包括：

- 无效 `dump` 源码或错误的 `--impl` 顶层项数量；
- 规则文件发现、读取、YAML 解析、校验或编译失败；
- 无效匿名 pattern 或 guard 匹配器编译；
- 扫描目标文件系统、文件读取或其他运行期失败。

扫描期间遇到的 MoonBit 源码解析失败会产生 warning，并返回状态 0。
