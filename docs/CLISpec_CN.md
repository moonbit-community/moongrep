...
规则 id 进行精确比较。每个请求的 id 都必须存在；第一个未知 id 会成为状态码 2
的用法错误。
...
每个 `--exclude` 值都会被规范化：先删除末尾的 `/` 字符但保留单独的 `/`，再删除
所有开头的 `./`。例如，`./vendor/` 变成 `vendor`，
...
任一个完整字符串匹配都会跳过该文件或目录条目。比较把 glob 字符和 `..` 当作
普通文本，保留文件系统路径拼写和大小写。因为每一层都进行条目名称匹配，排除
`vendor` 会跳过每个名为 `vendor` 的子条目，排除 `generated.mbt` 会跳过每个同名
子文件。`src/generated` 这样的路径只匹配构造路径拼写与它完全相同的子条目。
排除只应用于目录遍历发现的子条目；显式选择的扫描根仍会被检查。
