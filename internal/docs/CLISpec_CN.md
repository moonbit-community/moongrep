...
目录身份通过 `realpath` 解析，每次扫描中每个解析后的目录最多进入一次。符号链接
环和后续指向已访问目录的别名都会被跳过。

### 默认目录排除

在考虑子条目时，moongrep 会在每一层应用以下默认排除规则：
...
点前缀规则不涉及大小写。其他完整条目名称在 Linux 和 macOS 上区分大小写；
仅在 Windows 上，它们按逐字符 Unicode 小写映射进行不区分大小写的比较。
这些排除应用于遍历中的子条目。显式选择的扫描根即使最后一个路径分量匹配其中一条
规则，也仍会被检查。
...
记录只包含上面列出的字段，字段顺序为 `type`、`kind`、`content`。`kind` 反映所选
