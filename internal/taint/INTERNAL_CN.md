...
本文记录修改 `internal/taint` 各包及其规则集成时容易遗漏的实现细节。它面向
analyzer 及其调用方的维护者，不面向规则作者。

## 包职责

根 `internal/taint` 包是基于 MoonBit parser CST 的通用过程内污点分析引擎兼容门面。
它的生产源码只包含三个 `pub using` 块。实现拆分为：

- `internal/taint/domain`：存储路径、值位置、origin、相对污点、sink 标识和 sink finding
- `internal/taint/model`：规范化调用、transfer、matcher、effect 和规则配置
- `internal/taint/engine`：公开分析结果和入口，以及私有状态、CST helper 和求值实现
...
1. `internal/rule/compile` 校验 taint clause 并记录 `TaintTarget`。
2. `matching` 匹配编译后的 source、sink 和 sanitizer call pattern。
3. `internal/rule/taint_lowering` 构造带 `custom_transfer_call` 的 `TaintSpec`。
4. `internal/rule/apply` 运行 `@taint.analyze_function_like`，并把 sink finding 映射为 scanner `RuleFinding`。
...
已编译 sink 数组里的零基位置。`internal/rule/apply` 只接受小于计划 sink 数量的非负 pattern
index；命名、负数或越界 ID 会抛出内部 apply 错误，不会产生 finding，也不会回退到
pattern `0`。

`internal/rule/compile` 层保证 `__SOURCE__` 在 sink 或 sanitizer 中恰好出现一次，
并且是完整 receiver 或完整 argument。正因如此，`internal/rule/taint_lowering` 可以直接从
...
3. 在 `internal/taint/engine/state_wbtest.mbt` 中添加 read、write、kill 和 merge 测试
...
4. 添加直接的 `internal/taint/taint_test.mbt` 覆盖
5. 如果 YAML 行为变化，添加 `internal/rule/apply` 或 `internal/rule/taint_lowering` 测试

修改 YAML taint 行为时：

1. 更新 `internal/rule/compile` 中的校验
2. 更新 `internal/rule/taint_lowering` 中的 target 提取或 transfer 生成
3. 如果 `__SOURCE__` 匹配行为变化，保持 `internal/matching/INTERNAL.md` 的占位符说明同步
