...
changing the `internal/matching` package. It is for maintainers of the matcher and its
...
The public `model` package defines `BoundValue` as either `Single(CstNode)` or
...
Indexed matches also return identifier capture provenance separately from
`BoundValue`. The first successful capture records its source role and binding;
typed identifier ellipses extract these from each complete captured list item.
Inherited captures constrain each occurrence's binding when both occurrences
are lexical variables. New captures within the same shape retain ordinary
name or structural equality. Provenance is copied, committed, and rolled back
with capture branches, so binding mismatches can retry atomic ellipsis splits.
...
- `internal/rule/compile/compile.mbt` validation for reserved names and supported
...
5. add focused tests in `internal/matching/matching_test.mbt`
6. add `internal/rule/apply` or taint integration tests if compiled rule behavior
   changes

For matcher-only changes, `moon test internal/matching/matching_test.mbt` is the tight
